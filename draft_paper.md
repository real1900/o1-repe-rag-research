...
5. **Mahalanobis Distance Triage:** Finally, we upgraded the unsupervised distractor triage layer. By replacing the naive Cosine Similarity (which assumes a spherical latent cluster) with Mahalanobis Distance, the system evaluates the geometric out-of-distribution severity based on the elliptical *variance* of the context cluster via the Inverse Covariance Matrix. This exponentially isolated the distractor vectors (Distance Delta shifting from 0.15 to computationally massive deviations $>4,400$), guaranteeing zero-shot distractor triage without external Cross-Encoders.
