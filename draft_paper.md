...
1. **Real-World Distractor Identification via Two-Stage Filtering:** This study leveraged HotpotQA's ground truth to cleanly extract distractor signatures. In production, the LLM does not inherently know which retrieved chunk is the distractor. To resolve this, the architecture should employ a fast Cross-Encoder (e.g., `BGE-Reranker`) as a Stage 1 triage. If the reranker flags a chunk with a low semantic score, the chunk is not discarded; instead, it is passed through our $O(1)$ RepE framework to generate a Negative Control Signature. These mathematically extracted vectors act as subliminal guardrails, steering the generative LLM away from the flagged chunks without requiring autoregressive critique.
2. **Preventing Semantic Collapse via KL-Divergence Braking:** While our $\alpha$ probe predicts the geometric sweet spot, language is fluid. If the steering vector overwrites the model's core logic, semantic collapse occurs. Future work should implement a dynamic braking system that calculates the Kullback-Leibler (KL) Divergence between the unsteered and steered token probability distributions during inference. If the divergence crosses a catastrophic threshold, the system instantly decays the $\alpha$ coefficient for that specific decoding step, preventing the sentence from shattering.
3. **Scaling to High-Baseline Models via Targeted Layer Steering:** To prove the mathematical theorem of $O(1)$ latency, we utilized a lightweight model (GPT-2) with a 4.00% baseline. Applying this technique natively to state-of-the-art models (e.g., Llama-3, 70B MoE) risks overwriting their superior baseline reasoning capacity. Because LLMs sequester different operations across their depth (middle layers handle factual retrieval; deep layers handle logical grammar), the Negative Vector subtraction should only be injected into specific "knowledge retrieval" layers. This targeted layer steering neutralizes hallucinatory facts while leaving the model's flawless linguistic logic completely untouched.
