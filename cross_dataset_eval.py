"""Cross-dataset evaluation: run the headline MACH-1 config (Llama-3.2-3B,
layer 7, α=2, unit-direction contrastive) on TriviaQA-RC and 2WikiMultihopQA,
each with paired baseline + 5 random-direction controls.

The dataset is selected via DATASET env var: "triviaqa" or "twiki".
This is the same harness as random_direction_control.py but parametrized for
different datasets so the existing analysis pipeline applies unchanged.
"""
import os, json, time
import numpy as np
import torch
...
DATASET = os.environ.get("DATASET", "twiki").lower()
N = int(os.environ.get("N_QUERIES", 500))
...
DATASET_FILES = {
    "triviaqa": "triviaqa_filtered_5000.json",
    "twiki":    "twiki_filtered_5000.json",
    "hotpot":   "hotpot_filtered_5000.json",
}
INPUT = DATASET_FILES[DATASET]
print(f"[config] dataset={DATASET}  layer={TARGET_LAYER}  α={ALPHA}  n={N}  seed={SEED}")
...
print(f"[data] {len(eval_set)} queries from {INPUT}")

# Pre-sample 5 fixed random directions
...
t_start = time.time()
print(f"[eval] starting on {len(eval_set)} queries with {len(CONDS)} conditions...")
...
for i, row in enumerate(eval_set):
    ...
    v_contr = v_neg - v_pos
    n_v = torch.linalg.norm(v_contr)
    unit_contr = v_contr / n_v if n_v.item() > 1e-6 else torch.zeros_like(v_contr)
    ...
    if (i + 1) % 25 == 0:
        ...
        print(f"  [{i+1}/{len(eval_set)}] elapsed={elapsed:.0f}s  base={b:.3f}  contrastive={cn:.3f}  random_avg={rmean:.3f}")

results["meta"] = {
    "model": MODEL, "dataset": DATASET, "n_queries": len(eval_set),
    "target_layer": TARGET_LAYER, "alpha": ALPHA, "seed": SEED,
    "total_seconds": time.time() - t_start,
}
out_path = f"per_query_outcomes_cross_{DATASET}.json"
...
print(f"\n  random_dir mean (5 seeds): {100*np.mean(random_ems):.2f} ± {100*np.std(random_ems):.2f}")
print(f"  contrastive vs random mean: Δ={100*(np.asarray(results['contrastive']['correct']).mean() - np.mean(random_ems)):+.2f} pp")

stat_path = f"statistical_summary_cross_{DATASET}.json"
