...
# =================================================================================
# Llama-3.2-1B-Instruct Evaluation Pipeline
...
# Using Llama 3.2 1B Instruct as the proxy for high-baseline models
...
try:
    ...
    print("\nSkipping Llama-3 evaluation. You likely need to run `huggingface-cli login` to access Meta models.")
    exit(1)
...
def format_llama_prompt(distractor, question):
    # Use Llama-3 instruction format
    return f"<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\nContext: {distractor}\n\nQuestion: {question}\nAnswer purely based on the context.<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
...
def evaluate_llama():
    ...
    # We use a static calibrated alpha for this proof of concept since the Linear Probe is 
    # calibrated specifically to GPT-2's 12-layer embedding space.
    STATIC_LLAMA_ALPHA = 0.35 
    ...
    for i, row in enumerate(eval_set):
        ...
        prompt_text = format_llama_prompt(distractor, question)
        ...
        register_targeted_hooks(STATIC_LLAMA_ALPHA, c_vecs_by_layer)
        with torch.no_grad():
    ...
    if base_acc > 0:
        ...
        print(f"\nTargeted Layer Steering successfully delivered a +{relative_jump:.2f}% Relative Improvement on a High-Baseline model.")
    print("==========================================================")
