...
\input{layer7_validation_table.tex}
...
\input{llama3_8b_results_table.tex}
...
\input{random_control_table.tex}
...
As established in Sec.~\ref{sec:method-component3}, the token-norm distribution in Table~\ref{tab:token-norms} does \emph{not} support a clean ``factual vs.\ grammatical'' split---function words such as ``is'' and ``located'' carry the largest norms, not named entities. What the table does show is that all mid-sequence tokens sit well above the threshold $\tau\!=\!15$, while the BOS/sentinel position collapses far below it. The gating function (Eq.~\ref{eq:gating}) therefore acts as a boundary-condition safety net---suppressing steering only at norm-collapsed sentinel positions---and its load-bearing role is on heavily-aligned models, where it composes with KL-divergence braking to interrupt the collapse mode.
...
\begin{itemize}
    \item \textbf{Single-shot retrieval.} The pipeline assumes a fixed retrieved set; multi-step retrieval with adaptive query rewriting is not modeled.
    \item \textbf{Small, scale-dependent effect.} The positive effect is modest (a $+1.0$\,pp headline gain, borderline-significant) and appears only on Llama-3.2-3B-Instruct; it does not transfer to GPT-2 or Llama-3.1-8B-Instruct under the configurations tested. We do not claim a scale-robust improvement.
    \item \textbf{Distractor-centric evaluation.} Each prompt pairs the question with a distractor passage; mixed gold-plus-distractor retrieval sets, multi-document evidence aggregation, and contradictory-evidence settings are not evaluated.
    \item \textbf{Layer selection not swept on every model.} The layer-position sweep was conducted on Llama-3.2-3B; GPT-2 used a fixed mid-layer target, so a steering-receptive early layer there cannot be ruled out.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Realistic retrieval settings.} Evaluate on mixed gold-plus-distractor retrieval sets and multi-document evidence aggregation, rather than single-distractor prompts.
    \item \textbf{Larger and MoE models.} Extend the layer-position analysis to Llama-3-70B-class and mixture-of-experts architectures with re-calibrated $\tau$.
    \item \textbf{Adversarial co-training.} Train a distractor-injecting adversary against the triage layer to characterize the decision-boundary frontier.
...
This paper introduced MACH-1, a constant-time mechanistic intervention for retrieval-augmented hallucination suppression, and---more importantly---we evaluated it rigorously enough to find the limits of the natural design. By composing contrastive activation extraction, automated Logit Lens layer selection, residual-norm token gating, hybrid multi-dimensional triage, and a hybrid coefficient predictor, MACH-1 expresses the disambiguation step as a single tensor subtraction inside the LLM's residual stream---in principle replacing Self-RAG's $O(N)$ autoregressive critique loop with $O(1)$ vector arithmetic. We evaluated the system across three model scales (GPT-2 124M, Llama-3.2-3B-Instruct, and Llama-3.1-8B-Instruct), with bootstrap 95\% confidence intervals on Exact Match and paired permutation $p$-values for every variant against Blind RAG. The structural $O(1)$ vs.\ $O(N)$ asymmetry is supported empirically by per-query timings and by a head-to-head against a Self-RAG-style critique protocol (Sec.~\ref{sec:results-selfrag-h2h}).
...
All code, per-query outcome JSONs, bootstrap/permutation seeds (master seed: 2026), the synthetic alpha-tuning dataset, the trained MLP probe weights, the IEEE LaTeX source of this paper, and the accompanying executable Jupyter notebook are released alongside the paper. The notebook reproduces the GPT-2 6-condition ablation end-to-end on Apple Silicon MPS. The Llama-3.2-3B and Llama-3.1-8B experiments are reproducible via the per-experiment driver scripts in the repository---\texttt{llama3\_layer7\_validate.py}, \texttt{llama3\_layer\_sweep.py}, \texttt{llama3\_ablation\_v3.py}, \texttt{cross\_dataset\_eval.py}, \texttt{llama8b\_headline.py}, \texttt{random\_direction\_control.py}, \texttt{selfrag\_baseline.py}, and \texttt{actual\_selfrag.py}---each runnable on a single 24\,GB GPU or an Apple-Silicon MPS device; full pipeline $\sim$\$0.22 in compute at \$0.36/hr. All evaluation queries are drawn from the publicly available HotpotQA distractor split.
